...
from models.mnist_model import ClientNet, ServerNet
...
def run_experiment():
    ...
    server_train_data = datasets.MNIST(data_dir, train=True, download=True, transform=transform)
    server_test_data = datasets.MNIST(data_dir, train=False, transform=transform)

    client1_train_dataset = datasets.MNIST(data_dir, train=True, download=True, transform=transform)
    client1_test_dataset = datasets.MNIST(data_dir, train=False, transform=transform)
    client1_train_dataset.data = client1_train_dataset.data[:, :, :div]  # torch.Size([60000, 28, 10])
    client1_test_dataset.data = client1_test_dataset.data[:, :, :div]  # torch.Size([10000, 28, 10])

    client2_train_dataset = datasets.MNIST(data_dir, train=True, download=True, transform=transform)
    client2_test_dataset = datasets.MNIST(data_dir, train=False, transform=transform)
    client2_train_dataset.data = client2_train_dataset.data[:, :, div:]  # torch.Size([60000, 28, 18])
    client2_test_dataset.data = client2_test_dataset.data[:, :, div:]  # torch.Size([10000, 28, 18])
    ...
    server_loss_func = nn.CrossEntropyLoss().to(device)
    ...
    client1_model = ClientNet(1*28*10).to(device)
    ...
    client2_model = ClientNet(1*28*18).to(device)
